...
_INSTRUMENTED_ENGINES: WeakKeyDictionary[Engine, bool] = WeakKeyDictionary()
...
def instrument_engine(engine: Engine, *, logger: ComponentLoggerInterface | None, config: LoggingConfig | None = None) -> None:
    ...
    if _INSTRUMENTED_ENGINES.get(engine):
        return
    _INSTRUMENTED_ENGINES[engine] = True
    ...
    def after_cursor_execute(
        ...
        should_log_query = logging_config.log_successful_queries
        should_log_slow = (
            logging_config.slow_query_threshold_ms is not None and duration_ms >= logging_config.slow_query_threshold_ms
        )
        ...
        logger.event(
            ...
            level=level,
            context=_query_context(engine, statement, parameters, duration_ms, cursor.rowcount, executemany, logging_config),
        )
    ...
    def begin(conn: Connection) -> None:
        logger.event(COMPONENT, "db_transaction_begin", "Database transaction started.", level=DEBUG, context=_engine_context(engine))

    @event.listens_for(engine, "commit")
    def commit(conn: Connection) -> None:
        logger.event(COMPONENT, "db_transaction_commit", "Database transaction committed.", level=DEBUG, context=_engine_context(engine))

    @event.listens_for(engine, "rollback")
    def rollback(conn: Connection) -> None:
        logger.event(COMPONENT, "db_transaction_rollback", "Database transaction rolled back.", level=DEBUG, context=_engine_context(engine))

    if logging_config.log_pool_events:
        _instrument_pool(engine, logger)
...
def _instrument_pool(engine: Engine, logger: ComponentLoggerInterface) -> None:
    @event.listens_for(engine.pool, "connect")
    def connect(dbapi_connection: Any, connection_record: Any) -> None:
        logger.event(COMPONENT, "db_pool_connect", "Database pool opened a connection.", level=DEBUG, context=_pool_context(engine, "connect"))

    @event.listens_for(engine.pool, "checkout")
    def checkout(dbapi_connection: Any, connection_record: Any, connection_proxy: Any) -> None:
        logger.event(COMPONENT, "db_pool_checkout", "Database pool checked out a connection.", level=DEBUG, context=_pool_context(engine, "checkout"))

    @event.listens_for(engine.pool, "checkin")
    def checkin(dbapi_connection: Any, connection_record: Any) -> None:
        logger.event(COMPONENT, "db_pool_checkin", "Database pool checked in a connection.", level=DEBUG, context=_pool_context(engine, "checkin"))
